...
|---|---|---|
| **Backend API** | Xử lý nghiệp vụ, tính phí, quản lý slot | .NET 8, Clean Architecture, EF Core, CQRS (Mediator) |
| **Web Admin (React)** | Quản trị viên/nhân viên bãi xe vận hành | React + Vite + Tailwind + SignalR |
...
│   ├── ParkingPro.Application/         # CQRS Commands/Queries, DTOs, Validators
│   ├── ParkingPro.Infrastructure/      # EF Core, Repositories, SignalR Hub, Payment gateway
│   ├── ParkingPro.API/                 # Controllers, Middleware, JWT, DI setup
│   └── ParkingPro.Shared/              # Constants, Common utils
├── tests/
│   ├── ParkingPro.UnitTests/
│   └── ParkingPro.IntegrationTests/
├── parkingpro-admin-web/               # React Admin (Vite + Tailwind + SignalR client)
└── parkingpro-mobile/                  # React Native app (Customer + Staff)
```

**Pattern áp dụng (kế thừa kinh nghiệm CarBookingApp/BookingApp):**
- Clean Architecture, source-generated Mediator (martinothamar) cho CQRS
- FluentValidation cho pipeline behavior
...
- Theo phong cách đã dùng ở các dự án trước: **dark theme**, dashboard-style, font monospace (JetBrains Mono) cho số liệu
- Nhãn giao diện tiếng Việt, code/namespace tiếng Anh
- Trang chính Admin Web: **Sơ đồ bãi xe realtime** (grid slot, màu theo trạng thái) + **Dashboard doanh thu**
